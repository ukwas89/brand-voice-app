                        ...
                        if tag.name.startswith('h'):
                            ...
                            if heading['level'] == 2:
                                content_structure.append({
                                    'type': 'heading',
                                    'level': 3,
                                    'text': f"Detailed insight into {heading['text']}"
                                })
                                content_structure.append({
                                    'type': 'paragraph',
                                    'text': f"This subsection provides deeper insights into the aspect of: {heading['text']}."
                                })
                        else:
                            content_structure.append({
                                'type': 'paragraph',
...
st.markdown("""
    Transform complex content into clear, accessible UK English with structural integrity and clarity.
    """)
        ...
        if st.button("Generate Simplified Content", use_container_width=True):
            if not website_url.startswith('http'):
                ...
                with st.spinner("Initializing content analysis..."):
                    ...
                    if crawled_data:
                        with st.spinner("Rewriting content in professional UK English..."):
